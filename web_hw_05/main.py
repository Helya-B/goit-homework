...
class PrivaBankApiService:
    ...
    async def __fetch_exchange_rate(self, date):
    ...
    async def get_exchange_rates(self, days):
        ...
        for i in range(days):
            ...
            tasks.append(self.__fetch_exchange_rate(date))
...
def format_exchange_rates(exchange_rates):
    ...
    for i, rate in enumerate(exchange_rates):
        date = (datetime.now() - timedelta(days=i)).strftime('%d.%m.%Y')
        formatted_rate = {
            date: {'EUR': {'sale': rate['exchangeRate'][0]['saleRateNB'],
                           'purchase': rate['exchangeRate'][0]['purchaseRateNB']},
                   'USD': {'sale': rate['exchangeRate'][1]['saleRateNB'],
                           'purchase': rate['exchangeRate'][1]['purchaseRateNB']}}}
...
async def main():
    ...
    async with PrivaBankApiService() as api_service:
        try:
            exchange_rates = await api_service.get_exchange_rates(days)
